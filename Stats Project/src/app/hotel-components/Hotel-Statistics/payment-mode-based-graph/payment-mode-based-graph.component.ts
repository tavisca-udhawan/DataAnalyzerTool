...
export class PaymentModeBasedGraphComponent implements OnInit {
  ...
  chart: string;
  paymentModeGraph: any
  defaultGraphType: string
    ngOnInit(){
      this.paymentModeGraph = null;
    ...
    GraphSelect(graphValue)
    {
      ...
      DisplayGraph(chart ) {
        ...
        //alert(graphType);
        if(this.paymentModeGraph!=null)
        {this.paymentModeGraph.destroy();}
         this.paymentModeGraph = new Chart('payment-mode-chart', {
          type: chart,
          data: {
